...
def rotation_switch(instance, s):
...
def height_constraints(instance, solver):
    min_height = instance.get_min_height()
    max_height = instance.get_max_height()
    height = instance.get_height()

    solver.add(min_height <= height)
    solver.add(height <= max_height)

    # Objective Function
    solver.minimize(height)


def check_rot_flags(instance, solver):
    ...
    for cir in range(instance.get_n_circuits()):
        ...
        # do not allow rotation if rectangle height (width) is larger than strip width (height)
        s.add(Implies(instance.get_circuit(cir)[0] > instance.get_height(), Not(rot_flags[cir])))

        # force rotation if rectangle width (height) is larger than strip width (height)
        s.add(Implies(instance.get_circuit(cir)[1] > instance.get_height(), rot_flags[cir]))


# domain constraint
def set_domain_constraints(instance, solver):
    ...
    for cir in range(instance.get_n_circuits()):
        ...
        s.add(corners[cir][1] + heights[cir] <= instance.get_height())
        ...
        if not instance.get_rotation() and simmetry_breaking:
            ...
            if cir == widest_idx:
                ...
                s.add(corners[cir][1] <= (instance.get_height() - heights[cir])/2)
            else:
                ...
                # same, but with height. We use implication because the height is an encoded variable
                s.add(Implies(instance.get_circuit(cir)[1] > (instance.get_height() - instance.get_circuit(widest_idx)[1])/2, ud[cir][widest_idx] == False))
  

# non-overlapping constraint
def set_non_overlap_constraints(instance, s):
    ...
    for cir in range(instance.get_n_circuits()):
        ...
        for j in range(cir+1, instance.get_n_circuits()):
            ...
            if simmetry_breaking:
                ...
                s.add(Implies(heights[cir] + heights[j] > instance.get_height(), ud[cir][j] == False))
                s.add(Implies(heights[j] + heights[cir] > instance.get_height(), ud[j][cir] == False))